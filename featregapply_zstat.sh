...
if { [ lindex $argv 0 ] == "" } {
    puts "
Usage: featregapply <feat_directory> \[-f\] \[-c\] \[-l <image>\] \[-s <image>\]
...
for { set i 1 } { $i < [ llength $argv ] } { incr i 1 } {
    if { ! [ string compare [ lindex $argv $i ] "-f" ] } {
      ...
      fsl:exec "/bin/rm -rf reg_standard"
      exit 0
    } elseif { ! [ string compare [ lindex $argv $i ] "-l" ] } {
      incr i 1
...
if { [ info exists fmri(inmelodic) ] && $fmri(inmelodic) } {
    if { $fmri(regstandard_res)>0.0001 } {
    ...
    fsl:exec "${FSLDIR}/bin/fslmaths reg_standard/filtered_func_data -Tstd -bin reg_standard/mask -odt char"
} else {

    foreach input [ imglob example_func mean_func ] {
	runflirt $input $REF $input example_func spline float
    }
    foreach input [ imglob stats/cope* stats/varcope* stats/tdof_t* stats/zstat* ] {
